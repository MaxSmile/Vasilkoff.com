    ...
    return (
        <div className={`project ${activeTab === 'all' || obj.categories?.includes(activeTab) ? 'block' : 'hidden'}`}>
            <Link href={"/portfolio/" + obj.slug} aria-label={obj.title} className="grayscale hover:grayscale-0">
                <div className="relative rounded-3xl border border-transparent bg-white drop-shadow-[5px_10px_80px_rgba(119,128,161,0.15)] transition duration-500 hover:border-secondary hover:bg-primary/20 ">
